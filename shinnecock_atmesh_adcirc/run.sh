...
tolerance=
for iCnt in "${MY_TOLERANCE}" "${defTolerance}"
do
  if $( isPosNumber "${iCnt}" ); then
    tolerance="${iCnt}"
    break
  fi
done
...
prog_name=mpirun
  progMPI="$( getPROG ${prog_name} "${binDir} ${defBinDir}" )"
  errstat=$?
...
#     "binDIr" directory.
prog_name=adcprep
  progPREP="$( getPROG ${prog_name} "${binDir} ${defBinDir}" )"
  errstat=$?
  if [ ${errstat} -ne 0 ]; then
    procError "The executable \"${prog_name}\" is not found" \
              "This is a required program for this test."
...
prog_name=NEMS.x
  progNEMS="$( getPROG ${prog_name} "${binDir} ${defBinDir}" )"
  errstat=$?
  if [ ${errstat} -ne 0 ]; then
    procError "The executable \"${prog_name}\" is not found" \
              "This is a required program for this test."
...
####################
### Get the number of cores for each requested modeling component

# This is for the total number of cores requested for this NEMS application
nProcs=0

### Atmospheric Model
getNemsModelVars ATM atmesh
  nProcs=$(( ${nProcs} + ${ATM_nProcs} ))

### Ocean Model
getNemsModelVars OCN adcirc
  nProcs=$(( ${nProcs} + ${OCN_nProcs} ))

### Wave Model
#getNemsModelVars WAV
#  nProcs=$(( ${nProcs} + ${WAV_nProcs} ))

### Ice Model
#getNemsModelVars ICE
#  nProcs=$(( ${nProcs} + ${ICE_nProcs} ))

### Hydrologic Model
#getNemsModelVars NWM
#  nProcs=$(( ${nProcs} + ${NWM_nProcs} ))

####################


### Files to compare upon model run completion
chkFiles=( fort.61.nc  fort.63.nc  fort.64.nc  maxele.63.nc  maxvel.63.nc )
nFiles=${#chkFiles[@]}
...
#----------
echo -n "    Preparing ADCIRC for this case..."

${progPREP} --np ${OCN_nProcs} --partmesh >  adcprep.log 2>&1
errstat=$?
if [ ${errstat} -ne 0 ] ; then
  echo "    ${progPREP} Exit Code: ${errstat}"
  procError "error in ${progPREP} for --partmesh" \
            "see the log file adcprep.log"
fi

${progPREP} --np ${OCN_nProcs} --prepall  >> adcprep.log 2>&1
errstat=$?
if [ ${errstat} -ne 0 ] ; then
  echo "    ${progPREP} Exit Code: ${errstat}"
  procError "error in ${progPREP} for --prepall" \
            "see the log file adcprep.log"
fi
...
${progMPI} --allow-run-as-root -np ${nProcs} ${progNEMS} > nems_run.log 2>&1
errstat=$?
if [ ${errstat} -ne 0 ] ; then
  echo "    NEMS exit code: ${errstat}"
  procError "error in running NEMS.x" \
            "see the log file nems_run.log"
fi
